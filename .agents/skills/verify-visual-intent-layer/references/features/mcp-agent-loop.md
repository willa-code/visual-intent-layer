...
The product reaches an agent through its MCP entry tools over a real standard-input transport against the built server. The agent opens a review, reads intent status for a delivered batch, and acknowledges it. Acknowledgement is not implementation and not verification.
...
- The agent calls `get_intent_status` for a delivered batch.
...
- **List the tools.** Run `… lever.mjs mcp --run <name> --tool open_visual_review --args '{"kind":"saved-html","path":"fixtures/gallery.html","waitMs":1}'`. Exit `0`; `tools` names `open_visual_review`, `get_intent_status` and `acknowledge_intent`, and the call returns a review URL and session identity.
