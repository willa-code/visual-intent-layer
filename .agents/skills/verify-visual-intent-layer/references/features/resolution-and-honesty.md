...
- **Reload marks a Pass ready only on a new revision.** Send an Annotation, then run `… lever.mjs reload` with the Artifact unchanged: `state` still reports the Pass `in-flight`. Change the file, run `… lever.mjs wait --target ".banner"`, then `… lever.mjs reload`: `state` reports the Pass `ready` with the new `toRevision`.
- **A second tab never moves the first tab's claim.** Not drivable with the Lever, which owns exactly one page: open the same review URL in two tabs, reload one onto a new revision, and the other still reports the revision it is showing and stamps its next Annotation with that revision. Proven by `tests/browser-loop.test.ts` — *keeps a tab reporting the revision it is showing when a second tab adopts a newer one*.
