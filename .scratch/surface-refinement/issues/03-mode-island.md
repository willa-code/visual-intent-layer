...
- [ ] Exactly two mode tiles exist: point and box. Operating the artifact is the unarmed state, not a tile, and no `Pointer`, `Text`, `Region` or `Arrange` control remains
- [ ] Arming is by pointer and by `P` / `B`; `V` and `Escape` return to operating the artifact; arming an armed tile returns to operating
- [ ] An armed tile is visibly distinct without relying on colour alone, and the resting state is visibly the resting state — no tile lit
- [ ] The island is the only chrome over the artifact, sits at the stage's lower edge, and is never hidden — including while an anchored card is open, which is positioned so the two cannot overlap
- [ ] Arming a tile changes the cursor over the artifact, and returning to operating restores the artifact's own cursor
- [ ] Each tile is at least 24×24 in every state, including at rest, and carries an accessible name and a tooltip sentence
- [ ] Single-key shortcuts are ignored while focus is in a text field or contenteditable region, and the island stays pointer-reachable so focus in a field never strands the operator in a mode
- [ ] `Arrange` no longer exists as a mode. The `arrange` member of `LayerTool` and the code path it selects are removed here; the relational capability it carried moves to issue 18
- [ ] One first-run hint per artifact explains the two tiles and the gestures, appears once, and never returns
