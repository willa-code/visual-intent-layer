...
- [x] An `Amend` action is offered on an Annotation that has been delivered and is not yet verified, and nowhere else
- [x] Amending creates a new Annotation, marks the original `superseded`, and records the successor on the original
- [x] The original's note, targets, relationships, attachments and history are unchanged by an amendment
- [x] Only the effective successor is delivered, and it carries what it replaced; superseded ancestors are not delivered as separate instructions
- [x] The chain reads as one row, and the row shows what replaced what without requiring the history to be opened
- [x] The amendment is delivered with Steering Intent through the named delivery path, not by sending the whole queue
- [x] `PATCH` on an Annotation that is neither draft nor queued is refused, and the refusal states why
- [x] History for an in-place note change no longer records an event named `drafted`
...
Resolved 2026-09-17: done. Amend creates a successor, marks the original Replaced and delivers the successor as Steering Intent; `src/annotation` and `tests/browser-loop.test.ts` drive it. `surface-refinement-pass-a` ticket 02 renamed `Superseded` to `Replaced` without changing the mechanism.
