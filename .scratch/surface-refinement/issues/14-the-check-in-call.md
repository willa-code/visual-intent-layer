...
- [x] A session-scoped call returns new direction since a cursor, and does not require an `envelopeId`
- [x] The call returns, at minimum: newly delivered Annotations with their intent, amendments that superseded something, pending interruption requests, and the current state of Annotations the agent was given before
- [x] The call records agent contact as part of the same operation, so a read-back and a check-in cannot disagree
- [x] Contact is durable across a service restart, and the surface can state when the agent last checked
- [x] A pending interruption request is stored session-scoped, is returned by the call, and is never carried in a Visual Intent Envelope
- [x] An agent that was not holding a call when the Builder-Reviewer acted can retrieve what arrived, using only what the shipped tool descriptions tell it
- [x] The tool description tells an agent when to check in, and the call is discoverable without prior state
- [x] The envelope's `review-interruption` value is documented as reserved and never emitted, or removed by a recorded schema decision; no value in the published schema can be one the product is unable to produce
...
Resolved 2026-09-17: done. `check_in` is session-scoped, returns new direction since a cursor and records durable contact; `src/mcp/service.ts` and `src/service/check-in.ts`.

Deferred confirmation: the `Driven live` box is parked with the iteration's Verification Run; the browser loop drives Check-In delivery.
