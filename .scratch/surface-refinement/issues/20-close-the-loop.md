...
- [ ] Changing the artifact on disk leads, through the surfaced reload, to every Annotation written against an earlier revision being re-resolved against the new revision
- [ ] Each row states its own resolution outcome, and the four outcomes are distinguishable by label plus a non-colour cue
- [ ] An ambiguous resolution presents its candidates on the row and chooses none of them
- [ ] An unresolved target with no candidates is reported as deleted, and the row states that approval is blocked
- [ ] Re-resolution runs once per revision, and a second pass over the same revision does not repeat the work
- [ ] The surface states when re-resolution last ran, so a stale resolution is visible as stale
- [ ] A resolution never changes the artifact, and never changes an Annotation's note or targets
- [ ] Each re-resolved row can be compared before and after without changing surface state
