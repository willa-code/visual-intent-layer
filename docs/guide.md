...
The loop starts when an agent calls `open_visual_review`, or when you start a
session yourself — the three commands are just below.
...
Removal is the same act as registration, in the same place. Codex ships
`codex mcp remove visual-intent-layer` and Claude Code ships
`claude mcp remove visual-intent-layer`. `opencode mcp` has no remove verb, and
...
- `docs/pi-validation.md` — the live-pi checklist, awaiting a human run
