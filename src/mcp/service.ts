    ...
    return [
      {
        ...
        description:
          'Open the Visual Direction Loop for an agent-produced web interface and wait for the Builder-Reviewer. Use this when the human\u2019s intent is lossy in words alone: they name a visible thing by location ("that button", "the header", "below the form"), they compare visible things ("align these", "same size as that", "more space here"), a prose-only correction already missed its target once, or they want to verify a visual result rather than read a code diff. Pointing is the gesture: pointing at elements, dragging across exact words, boxing an area, and sending grounded Annotations. Stay in plain chat when the request names files, functions or behaviour precisely, or when no visible artifact exists yet. Works with a saved HTML artifact (kind "saved-html" with a filesystem path) or a running local React/Vite app (kind "react-vite-app" with a localhost URL). The default browser opens automatically on the machine running the service. When the human sends, this call returns one batch carrying each Annotation with its own identity, target evidence and note. Steering and interruption are seen at a Check-In, not mid-step: call check_in between your own steps to read new direction, an amendment, or a stop request. If this call returns without a batch, the Annotations stay queued durably and check_in reads them.',
        inputSchema: {
          type: 'object',
          required: ['kind'],
          properties: {
